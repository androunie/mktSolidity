...
contract Factory {
    ...
    constructor(string memory _ontologyURI) public { //string memory _ontologyURI
  ...
  function getAllAuctions() public view returns (address[] memory) {
    address [] memory arrayAuctions;
      for (uint i = 0; i < uniqueIdCounter; i++ ) {
          arrayAuctions[i] = address(auctionStore[i]);
      }
